...
from schemas import OrderStatusModel
...
async def get_user_orders(Authorize:AuthJWT=Depends()):
    """
        ...
        This list the orders made by currently logged in users
    """
...
async def get_specific_orders(id:int,Authorize:AuthJWT=Depends()):
    """
        ## Get a spesific order by the currently logged in user
...
async def update_order_status(id:int,order=OrderStatusModel,Authorize:AuthJWT=Depends()):
    ...
    usernaem=Authorize.get_jwt_subject()
    current_user=session.query(User).filter(User.username==usernaem).first()
...
@order_router.delete('/order/delete/{id}/',status_codee=status.HTTP_204_NO_CONTENT)
